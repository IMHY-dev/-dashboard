...
module.exports = {
  ...
  'ats-b': (args) => {
    ...
    return {
      ...
      예산연동: [
        { 구분: 'LE확인',
          from: { 신청월: mon, 코스트센터: args.from_cc ?? 'D2034', GL계정: args.from_gl ?? '343006', 금액: tot },
          to:   { 신청월: mon, 코스트센터: args.to_cc   ?? 'D2034', GL계정: args.to_gl   ?? '343006', 금액: tot } }
      ],
      본문: `1. 목적
...
인터뷰 대상자와의 긍정적 관계 형성을 통한 향후 재접점 기반 마련`,
  ...
  'survey-b': (args) => {
    ...
    return {
      ...
      예산연동: months.map(mon => ({
        구분: 'LE확인',
        from: { 신청월: mon, 코스트센터: args.from_cc ?? 'M2010', GL계정: args.from_gl ?? '343002', 금액: amt },
        to:   { 신청월: mon, 코스트센터: args.to_cc   ?? 'M2010', GL계정: args.to_gl   ?? '343002', 금액: amt },
      })),
      본문: `1. 배경 및 목적
...
기존에 하기 어려웠던 '세그먼트별 Share', 즉 지역, 연령, 연봉, 직무, 회사 특성별 경쟁사 대비 어떠한 위치에 있는지 알아냄으로써 전사 경영 의사결정에 기여할 것으로 기대합니다.`,
  ...
  'free-b': (args) => {
    ...
    const totalHours = n * weeks * hpw;             // 총 근무 시간
    const amt        = parseInt(args.amt ?? 5000000); // 총 지불 금액 (입력값)
    const amtMan     = Math.round(amt / 10000);
    const hourly     = Math.round(amt / totalHours); // 시급 = 총 지불 금액 / 총 시간 (결과값)
    return {
      기본: {
  ...
  'interview-b': (args) => {
    ...
    return {
      ...
      예산연동: [
        {
          구분: args.type_sap ?? '전용',
          from: { 신청월: fromMon, 코스트센터: args.from_cc ?? 'H2200', GL계정: args.from_gl ?? '343002', 금액: amt },
          to:   { 신청월: toMon,   코스트센터: args.to_cc   ?? 'H2200', GL계정: args.to_gl   ?? '343002', 금액: amt },
        }
      ],
      본문: `1. 배경 및 목적
