...
module.exports = {
  ...
  'ats-b': (args) => {
    ...
    return {
      ...
      예산연동: [
        { 구분: 'LE확인', from: { 신청월: mon, 코스트센터: args.cc ?? 'D2034', GL계정: args.gl ?? '343006', 금액: tot }, to: { 신청월: mon, 코스트센터: args.cc ?? 'D2034', GL계정: args.gl ?? '343006', 금액: tot } }
      ],
      본문: `1. 목적
...
인터뷰 대상자와의 긍정적 관계 형성을 통한 향후 재접점 기반 마련`,
  ...
  'survey-b': (args) => {
    ...
    return {
      ...
      예산연동: months.map(mon => ({
        구분: 'LE확인',
        from: { 신청월: mon, 코스트센터: args.cc ?? 'M2010', GL계정: args.gl ?? '343002', 금액: amt },
        to:   { 신청월: mon, 코스트센터: args.cc ?? 'M2010', GL계정: args.gl ?? '343002', 금액: amt },
      })),
      본문: `1. 배경 및 목적
  ...
  'interview-b': (args) => {
    ...
    return {
      ...
      예산연동: [
        {
          구분: args.type_sap ?? '전용',
          from: { 신청월: fromMon, 코스트센터: args.from_cc ?? 'M2010', GL계정: args.gl ?? '343002', 금액: amt },
          to:   { 신청월: toMon,   코스트센터: args.to_cc   ?? 'H2200', GL계정: args.gl ?? '343002', 금액: amt },
        }
      ],
      본문: `1. 배경 및 목적
